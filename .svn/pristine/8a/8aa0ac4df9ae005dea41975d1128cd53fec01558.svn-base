...
class Rememberme {
	...
	function setCookie($netid = "", $nocookie = false) {
		...
		$domain = ($_SERVER['SERVER_NAME'] == "localhost") ? false : $_SERVER['SERVER_NAME'];
		...
		if (!$nocookie) {
			...
			//die('   host    ' . $host . '   segments    ' . $segments . '   servername    ' . $_SERVER['SERVER_NAME']. '   domain    ' . $domain . '  path     ' . preg_replace('/^(http|https):\/\/(www\.)?' . $_SERVER['SERVER_NAME'] . '/', '', preg_replace('/\/$/','', base_url())));
